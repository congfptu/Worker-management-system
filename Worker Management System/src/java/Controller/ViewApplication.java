 ...
 * @author congfptu
 */
public class ViewApplication extends BaseController {
    ...
    protected void processGet(HttpServletRequest request, HttpServletResponse response)
       ...
       CongNhan cn = (CongNhan) request.getSession().getAttribute("congnhan");
        ...
        int num_records = ap.count(cn.getId());
        int totalpage = (num_records%5 ==0)
                ?num_records/5
                :(num_records/5)+1;
        ArrayList<Application> apps = ap.getAllApplicationsByUser(cn.getId(),pageindex,5);
        request.setAttribute("totalpage", totalpage);
        request.setAttribute("pageindex", pageindex);
        request.setAttribute("apps", apps);
        request.setAttribute("size", apps.size());
        request.getRequestDispatcher("view/viewapplication.jsp").forward(request, response);
    }
